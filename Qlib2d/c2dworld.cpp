        ...
        if (animation_id != id) {
            if (id == 1 || id == 2) {
                if (id == 1) {
                    ...
                    animation_queue.enqueue(QString::fromLocal8Bit(R"(map (\ `n `(box`(pos 0.0d 0.0d) `(size 0.04d 0.05d) `(mass 1d) `(text "矩"))) (range 0 10))"));
                    animation_queue.enqueue(QString::fromLocal8Bit(R"(map (\ `n `(circle`(pos 0.0d 0.0d) `(r 0.025d) `(mass 1d) `(text "圆"))) (range 0 5))"));
                    animation_queue.enqueue(QString::fromLocal8Bit(R"(map (\ `n `(tri`(pos 0.0d 0.0d) `(edge 0.04d 0.04d) `(angle 60d) `(mass 1d) `(text "角"))) (range 0 5))"));
                }
